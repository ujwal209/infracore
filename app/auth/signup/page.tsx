      ...
      <div className="hidden lg:flex w-[45%] fixed inset-y-0 left-0 bg-slate-900 flex-col justify-center p-12 xl:p-16 overflow-hidden z-10 border-r border-slate-800">
        ...
        <div className="relative z-10 w-full max-w-md mx-auto">
          ...
          <h1 className="text-4xl xl:text-5xl font-black text-white tracking-tighter uppercase leading-[1.1] mb-6">
            ...
            <span className="text-[#8B98FF] drop-shadow-[0_2px_15px_rgba(139,152,255,0.2)]">Future Node</span>
          </h1>
          ...
          <div className="space-y-4">
             <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors group">
                <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center shrink-0 border border-slate-700 group-hover:border-[#6B8AFF]/50 transition-colors">
                  <Layers className="text-[#6B8AFF]" size={16} />
                </div>
                <div>
        ...
        <div className="absolute bottom-10 left-10 xl:left-12 right-10 xl:right-12 z-10 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-slate-500 border-t border-slate-800 pt-6">
           ...
           <span className="flex items-center gap-2">
             <Fingerprint size={12} className="text-[#6B8AFF]"/> Vault Active
           </span>
        </div>
      </div>
                ...
                <>
                  ...
                  <div className="relative mb-6">
                    ...
                    <div className="relative flex justify-center text-[10px] font-black uppercase tracking-widest text-slate-400">
                      <span className="bg-white dark:bg-slate-900 px-4">Or continue with email</span>
                    </div>
                  </div>

                  <form className="space-y-6" onSubmit={handleSignupSubmit}>
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 ml-1">Commander Name</label>
                      <Input 
                        name="name" 
                        required 
                    ...
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 ml-1">Secure Password</label>
                      <div className="relative">
                        <Input 
                          ...
                          placeholder="••••••••" 
                        />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors p-1">
                    ...
                    <div className="space-y-2">
                      <label className="text-[11px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 ml-1">Verify Password</label>
                      <div className="relative">
                        <Input 
                          ...
                          placeholder="••••••••" 
                        />
                        <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors p-1">
                ...
                <form className="space-y-6 animate-in slide-in-from-right-8 duration-500" onSubmit={handleOtpSubmit}>
                  <div className="space-y-4 text-center">
                    <Input 
                      ...
                      maxLength={8}
                      required
                      className="h-24 text-center text-3xl sm:text-4xl font-mono font-bold tracking-[0.4em] pl-[0.4em] rounded-3xl bg-slate-50 dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 focus:bg-white dark:focus:bg-slate-900 focus-visible:ring-0 focus-visible:border-[#01005A] dark:focus-visible:border-[#6B8AFF] transition-all uppercase shadow-inner text-slate-900 dark:text-white" 
                      placeholder="--------" 
                    />
                    <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-4 flex items-center justify-center gap-1.5 flex-wrap">
                      <span>Sent to</span>
                      <span className="text-slate-900 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">{emailCache}</span>
                    </p>
                  </div>
                  ...
                  <Button 
                    ...
                    className="w-full bg-[#01005A] dark:bg-[#6B8AFF] text-white hover:bg-[#020080] dark:hover:bg-[#5274FF] font-black h-14 rounded-2xl uppercase mt-4 transition-all active:scale-[0.98] shadow-[0_4px_14px_rgba(1,0,90,0.25)] dark:shadow-[0_4px_14px_rgba(107,138,255,0.25)] tracking-widest text-xs"
                  >
                  ...
                  <button 
                    ...
                    className="w-full text-[11px] font-bold text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white uppercase tracking-widest transition-colors mt-6 pt-4 border-t border-slate-100 dark:border-slate-800"
                  >
              ...
              {step === 'form' && (
                <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 text-center">
                  <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center justify-center flex-wrap gap-2">
                    ...
                    <Link href="/auth/login" className="text-slate-900 dark:text-white hover:text-[#01005A] dark:hover:text-[#6B8AFF] transition-colors border-b border-transparent hover:border-[#01005A] dark:hover:border-[#6B8AFF] pb-0.5">
                      Access Terminal
                    </Link>
                  </p>
                </div>
