...
import { getBranchJobRoles, getAvailableBranches } from '@/app/actions/database/branch-details'
import { 
  Search, 
  ChevronLeft, 
  ChevronRight, 
...
export default function BranchRolesPage() {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      const { data: roles, count: totalCount } = await getBranchJobRoles({
        page,
        pageSize,
        search,
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-zinc-50 dark:bg-zinc-950 relative">
        ...
        <header className="h-16 flex-shrink-0 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 sm:px-8 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md z-20">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-xs sm:text-sm font-semibold uppercase tracking-wider truncate text-zinc-900 dark:text-zinc-100">Career Pathway</h2>
              <p className="text-[10px] font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider truncate">Market Demand Index</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 lg:p-8 space-y-6">
              ...
              <div className="relative w-full sm:w-64" ref={dropdownRef}>
                ...
                >
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="truncate">{selectedBranch === 'all' ? 'All Specialties' : selectedBranch}</span>
                  </div>
          ...
          {loading ? (
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
               {Array.from({ length: 12 }).map((_, i) => (
                 <div key={i} className="h-36 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl animate-pulse shadow-sm"></div>
               ))}
             </div>
          ) : data.length === 0 ? (
            <div className="h-40 flex flex-col items-center justify-center bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm gap-3 text-zinc-400 dark:text-zinc-500">
              <Briefcase size={32} className="opacity-20" />
              <span className="font-semibold text-xs tracking-wider uppercase">No Pathways Documented</span>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {data.map((role, idx) => (
                <div key={idx} className="bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm hover:shadow-md hover:border-blue-500/30 dark:hover:border-blue-500/50 transition-all duration-300 flex flex-col justify-between group h-full">
                  <div className="space-y-3">
                    <span className="inline-flex px-2 py-1 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded text-[9px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider line-clamp-1 break-all">
                      {role.branch_name}
                    </span>
                    <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 leading-snug group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2">
                      {role.job_role}
                    </h3>
                  </div>
                  <div className="mt-6 flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <span className="text-[10px] font-semibold text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5 uppercase tracking-wider">
                      <Activity size={12} className="text-blue-500 dark:text-blue-400" /> Active Demand
                    </span>
                    <div className="w-6 h-6 rounded-full bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center text-zinc-400 group-hover:bg-blue-50 dark:group-hover:bg-blue-500/10 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
          ...
          {!loading && data.length > 0 && (
            <div className="mt-6 border-t border-zinc-200 dark:border-zinc-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-[10px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                Showing <span className="text-zinc-900 dark:text-white">{(page - 1) * pageSize + 1}</span> to <span className="text-zinc-900 dark:text-white">{Math.min(page * pageSize, count)}</span> of <span className="text-zinc-900 dark:text-white">{count}</span> Roles
              </p>
