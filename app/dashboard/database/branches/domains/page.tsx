...
import { getBranchTechnicalDomains, getAvailableBranches } from '@/app/actions/database/branch-details'
import { 
  ChevronLeft, 
  ChevronRight, 
  Code, 
...
export default function BranchDomainsPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const { data: domains, count: totalCount } = await getBranchTechnicalDomains({
        page,
        pageSize,
    ...
    <div className="flex h-screen bg-zinc-50 dark:bg-zinc-950 overflow-hidden text-zinc-900 dark:text-zinc-100 font-sans antialiased w-full transition-colors duration-300 selection:bg-blue-500/30 selection:text-blue-900 dark:selection:text-blue-100">
          ...
          <div className="absolute left-0 top-0 h-full w-[85vw] max-w-sm bg-white dark:bg-zinc-950 shadow-2xl flex flex-col transform transition-transform duration-300 border-r border-zinc-200 dark:border-zinc-800" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-white dark:bg-zinc-950">
              <span className="font-semibold flex items-center gap-2 text-zinc-900 dark:text-white">
                <Code size={18} className="text-blue-600 dark:text-blue-500" /> Navigation
              </span>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-zinc-50 dark:bg-zinc-950 relative">
        ...
        <header className="h-16 flex-shrink-0 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 sm:px-8 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md z-20">
          <div className="flex items-center gap-3 min-w-0">
             ...
             <div className="min-w-0">
               <h2 className="text-xs sm:text-sm font-semibold uppercase tracking-wider truncate text-zinc-900 dark:text-zinc-100">Technical Lexicon</h2>
               <p className="text-[10px] font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider truncate">Branch Domain Index</p>
             </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 lg:p-8 space-y-6">
          ...
          {/* Action Header */}
          <div className="bg-white dark:bg-zinc-950 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-colors">
             ...
             <div className="relative w-full sm:w-72" ref={dropdownRef}>
                ...
                >
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span className="truncate">{selectedBranch === 'all' ? 'Filter Domain Index' : selectedBranch}</span>
                  </div>
             ...
             <button 
                ...
                title="Refresh Data"
              >
                <RefreshCcw size={16} className={loading ? 'animate-spin text-blue-600 dark:text-blue-400' : ''} />
              </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-48 bg-zinc-100 dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 animate-pulse shadow-sm"></div>
              ))
            ) : data.length === 0 ? (
               <div className="col-span-full h-40 flex flex-col items-center justify-center bg-white dark:bg-zinc-950 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm gap-3 text-zinc-400 dark:text-zinc-500">
                 ...
                 <span className="font-semibold text-xs tracking-wider uppercase">No Technical Signals Detected</span>
               </div>
            ) : (
              data.map((item, idx) => (
                <div key={idx} className="bg-white dark:bg-zinc-950 rounded-[2rem] p-6 sm:p-8 border border-zinc-200 dark:border-zinc-800 shadow-sm relative group hover:shadow-md hover:border-blue-500/30 dark:hover:border-blue-500/50 transition-all flex flex-col h-full">
                   
                   <div className="space-y-1.5 mb-6">
                      <span className="text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider flex items-center gap-1.5">
                        <Binary size={12} className="text-blue-500 dark:text-blue-400" /> Substrate Layer
                      </span>
                      <h3 className="text-lg sm:text-xl font-semibold text-zinc-900 dark:text-zinc-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-tight line-clamp-2">
                        {item.branch_name}
                      </h3>
                   </div>

                   <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800">
                      {item.technical_domains.split(',').map((domain: string, i: number) => domain.trim() && (
                        <span key={i} className="px-2.5 py-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider transition-colors cursor-default hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400">
                           {domain.trim()}
                        </span>
                      ))}
                   </div>
          ...
          {!loading && data.length > 0 && count > 0 && (
            <div className="mt-8 border-t border-zinc-200 dark:border-zinc-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-[10px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                Showing <span className="text-zinc-900 dark:text-white">{(page - 1) * pageSize + 1}</span> to <span className="text-zinc-900 dark:text-white">{Math.min(page * pageSize, count)}</span> of <span className="text-zinc-900 dark:text-white">{count}</span> Domains
              </p>
